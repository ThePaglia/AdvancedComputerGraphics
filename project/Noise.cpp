...
namespace procedural
{
    ...
    void Randomize(int seed)
    {
        if (seed != 0)
        {
            ...
            // with each byte
            uint8_t buffer[4];
            buffer[0] = (uint8_t)(seed & 0x00ff);
            buffer[1] = (uint8_t)((seed & 0xff00) >> 8);
            buffer[2] = (uint8_t)((seed & 0x00ff0000) >> 16);
            buffer[3] = (uint8_t)((seed & 0xff000000) >> 24);

            for (int i = 0; i < 256; i++)
            {
                _random[i] = Source[i] ^ buffer[0];
                _random[i] ^= buffer[1];
                _random[i] ^= buffer[2];
                _random[i] ^= buffer[3];
	...
	float getHeightOnUnitSphere(vec3 point) {
        return 1.0f - max(abs(get3DSimplexNoiseAtPointOnUnitSphere(point) * 0.3f), 0.01f);
	}
}